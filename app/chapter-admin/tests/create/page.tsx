...
return <Card><CardHeader><CardTitle>Create Test</CardTitle></CardHeader><CardContent className="space-y-4"><div><p className="text-sm mb-1">Event</p><Input value={eventName} onChange={(e)=>setEventName(e.target.value)} /></div><div><p className="text-sm mb-1">Pre-made Test</p><select className="w-full border rounded p-2" value={testName} onChange={(e)=>setTestName(e.target.value)}>{preMade.map(t=><option key={t}>{t}</option>)}</select></div><div><p className="text-sm mb-1">Assign Members</p><div className="max-h-52 overflow-auto border rounded p-2 space-y-2">{members.map(m=><label key={m.uid} className="flex items-center gap-2"><input type="checkbox" checked={!!selected[m.uid]} onChange={(e)=>setSelected((p)=>({...p,[m.uid]:e.target.checked}))} />{m.name}</label>)}</div></div><div><p className="text-sm mb-1">Deadline</p><Input type="datetime-local" value={deadlineAt} onChange={(e)=>setDeadlineAt(e.target.value)} /></div><Button onClick={submit}>Create Test</Button></CardContent></Card>}
